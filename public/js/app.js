...
app.controller("user-list", ["$scope", "$rootScope", "socket", ($scope, $rootScope, socket) => {
  $scope.users = [];
  $scope.title = "VChat";
  $scope.text;
  $scope.activeUser = {
  ...
  $scope.turnOnCamera = async () => {
    return new Promise((resolve, reject) => {
      navigator.webkitGetUserMedia(mediaStream, stream => {
  ...
  $scope.chat = ({name, id}) => {
    ...
    $scope.showChat = true;
    $scope.activeUser.chat = JSON.parse(JSON.stringify(chatDatabase[id] || []));
  };
  ...
  $scope.send = () => {
    ...
    if ($scope.text) {
      updateChatLog($scope.activeUser.id, "sent", {timestamp, text: $scope.text });
  ...
  let send = (to, type, payload) => {
    socket.emit("message", {to, type, payload});
    if (type == "text") {
      $(".chat-input")[0].value = "";
      $rootScope.$digest();
    }
